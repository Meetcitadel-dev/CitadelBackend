...
class WebSocketService {
    ...
    async updateUserOnlineStatus(userId, isOnline) {
        try {
            const existing = await userOnlineStatus_model_1.default.findOne({ userId });
            if (existing) {
                ...
                await userOnlineStatus_model_1.default.create({ userId, isOnline, lastSeen: new Date() });
            }
    ...
    async handleMarkAsRead(socket, data) {
        try {
            ...
            await message_model_1.default.updateMany({ conversationId, senderId: otherUserId, status: { $ne: 'read' } }, { $set: { status: 'read' } });
    ...
    emitConnectionRequest(targetUserId, requestData) {
    ...
    emitConnectionRequestAccepted(requesterId, acceptData) {
    ...
    emitConnectionRequestRejected(requesterId, rejectData) {
    ...
    emitConnectionRemoved(targetUserId, removeData) {
