    ...
    async getMatchedConversations(req, res) {
        try {
            ...
            const conversations = await Promise.all(matches.map(async (match) => {
                ...
                const hasChatHistory = conversation && await message_model_1.default.countDocuments({
                    conversationId: conversation._id
                }) > 0;
                // Determine the case
                ...
                const profileImage = await (async () => {
                    try {
                        // Prefer slot 0
                        const mapping = await userImageSlot_model_1.default.findOne({ userId: otherUser._id, slot: 0 });
                        let img = null;
                        if (mapping) {
                            img = await userImage_model_1.default.findById(mapping.userImageId);
                        }
                        // Fallback to first available slot
                        if (!img) {
                            const anyMapping = await userImageSlot_model_1.default.findOne({ userId: otherUser._id })
                                .sort({ slot: 1 });
                            if (anyMapping) {
                                img = await userImage_model_1.default.findById(anyMapping.userImageId);
                            }
                        }
                        if (!img)
                            return null;
