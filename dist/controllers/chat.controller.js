...
async function getSlot0ImageUrl(userId) {
    try {
        const mapping = await userImageSlot_model_1.default.findOne({ userId, slot: 0 });
        if (!mapping)
            return null;
        const img = await userImage_model_1.default.findById(mapping.userImageId);
        if (!img)
            return null;
...
class ChatController {
    ...
    async getActiveConversations(req, res) {
        try {
            ...
            const conversations = await Promise.all(connections.map(async (connection) => {
                ...
                if (conversation) {
                    const lastMsg = await message_model_1.default.findOne({ conversationId: conversation._id })
                        .sort({ createdAt: -1 })
                        .lean();
                    if (lastMsg) {
                ...
                let unreadCount = 0;
                if (conversation) {
                    unreadCount = await message_model_1.default.countDocuments({
            ...
            // Get user's groups
            const groupMemberships = await groupMember_model_1.default.find({ userId })
                .populate({
                path: 'groupId',
                match: { isActive: true }
            })
            ...
            const groupConversations = await Promise.all(groupMemberships
                .filter((membership) => membership.groupId) // Filter out null groups
                .map(async (membership) => {
                const group = membership.groupId;
                // Get last message
                const lastMessage = await groupMessage_model_1.default.findOne({ groupId: group._id })
                    .sort({ createdAt: -1 })
                    .populate('senderId', 'id name username')
                    .lean();
                // Get unread count - messages not in user's read list
                const readMessageIds = await groupMessageRead_model_1.default.find({
                    userId,
                    groupId: group._id
                }).distinct('messageId');
                const unreadCount = await groupMessage_model_1.default.countDocuments({
                    groupId: group._id,
                    senderId: { $ne: userId },
                    _id: { $nin: readMessageIds }
                });
                const memberCount = await groupMember_model_1.default.countDocuments({ groupId: group._id });
                return {
                    id: `group_${group._id}`,
                    groupId: group._id,
                    name: group.name,
    ...
    async sendMessage(req, res) {
        try {
            ...
            await unreadCount_service_1.default.updateDirectChatUnreadCount(otherUserId, userId, otherUserId, newMessage._id);
    ...
    async getConversationByUserId(req, res) {
        try {
            ...
            const profileImage = await getSlot0ImageUrl(targetUser._id);
            res.json({
                success: true,
                conversation: {
                    id: conversation._id,
    ...
    async getConversationDetails(req, res) {
        try {
            ...
            const otherUser = await user_model_1.default.findById(otherUserId)
                .select('id name username')
