...
class WebSocketDebugger {
    ...
    async testDirectEmission() {
        ...
        try {
            const response = await axios_1.default.post(`${API_BASE}/groups/${this.testGroupId}/messages`, { content: 'Direct WebSocket test message' }, {
                headers: {
            ...
            console.error('❌ API message failed:', ((_a = error.response) === null || _a === void 0 ? void 0 : _a.data) || error.message);
        }
        // Wait for WebSocket messages
        await this.wait(3000);
        console.log('✅ Direct emission test completed\n');
    }
    async testAPIMessage() {
        var _a;
        console.log('📤 Testing API message sending...');
        try {
            const response = await axios_1.default.post(`${API_BASE}/groups/${this.testGroupId}/messages`, { content: 'API test message from debug script' }, {
                headers: {
