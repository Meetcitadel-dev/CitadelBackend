...
async function debugInteractions() {
    try {
        ...
        const ishaInteractions = await interaction_model_1.default.findAll({
            where: {
        ...
        const ishaConnection = await connection_model_1.default.findOne({
            where: {
                [sequelize_1.Op.or]: [
        ...
        const blockedUserIds = blockedConnections.map(conn => conn.userId1 === yourProfile.id ? conn.userId2 : conn.userId1);
        console.log('   Blocked user IDs:', blockedUserIds);
        // Get interacted users
        const previousInteractions = await interaction_model_1.default.findAll({
            where: {
                userId: yourProfile.id,
                interactionType: {
                    [sequelize_1.Op.in]: ['viewed', 'connected', 'adjective_selected', 'blocked']
                }
            },
        ...
        const allUsers = await user_model_1.default.findAll({
            where: {
                id: {
                    [sequelize_1.Op.ne]: yourProfile.id,
                    [sequelize_1.Op.notIn]: [...blockedUserIds, ...interactedUserIds]
                },
